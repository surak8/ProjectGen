...
namespace NSprojectgen {
    class CProjectGenerator {
        ...
        internal static void generate(string filename, string version, string asmName, string ns, ProjectType type) {
            ...
            //         addLabeledGroup(p.Xml, "ExtensionSettings");
            addPropertySheetImports(p.Xml);
            //            addPropertyGroup(p.Xml, "UserMacros");
            ...
            //            addItemGroup(p.Xml, new Blah4(fakeC));
            //          addItemGroup(p.Xml, new Blah4(fakeNone));
            //        addItemGroup(p.Xml, new Blah4(fakeH));
            ...
            //    addImportGroup(p.Xml, "ExtensionTargets");
            p.Save(filename);
        }
        ...
        static void populateLinker(ProjectItemDefinitionElement avar, bool isDebug) {
            //        if (isDebug)
            //          avar.AddMetadata("GenerateDebugInformation", "true");
            //            else {
            //              avar.AddMetadata("EnableCOMDATFolding", "true");
            //            avar.AddMetadata("OptimizeReferences", "true");
            //      }
            //    avar.AddMetadata("OutputFile", "$(SolutionDir)$(TargetName)$(TargetExt)");
            //            if (isDebug)
            //              avar.AddMetadata("ProgramDatabaseFile", "$(SolutionDir)$(TargetName).pdb");
            if (!isDebug) {
                avar.AddMetadata("EnableCOMDATFolding", "true");
                avar.AddMetadata("OptimizeReferences", "true");
            }
            //      avar.AddMetadata("Subsystem", "Console");
            //            avar.AddMetadata("TargetMachine", "MachineX86");
            //            if (!isDebug)
        ...
        static void populateCompiler(ProjectItemDefinitionElement avar, bool isDebug) {
            ...
            //avar.AddMetadata("WarningLevel", isDebug ? "Level4" : "Level3");
            //         avar.AddMetadata("Optimization", isDebug ? "Disabled" : "MaxSpeed");
            ...
            //          } else {
            //          avar.AddMetadata("IntrinsicFunctions", "true");
            //    }
            //            avar.AddMetadata("CompileAs", "Default");
            if (isDebug) {
                avar.AddMetadata("DebugInformationFormat", "ProgramDatabase");
        ...
        static void createGlobals(string ns, ProjectType type, Project p, string label) {
            ...
            //       p0.AddProperty("ProjectGuid", "{" + guid.ToString().ToUpper() + "}");
