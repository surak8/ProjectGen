                    ...
                    if ((len = anArg.Length) >= 2) {
                        if (anArg[0] == '-' || anArg[0] == '/') {
                            switch (anArg[1]) {
                                case 'f':
                                    if (len > 2)
                                        asmName = anArg.Substring(2).Trim();
                                    else { asmName = args[i + 1]; i++; }
                                    //         filename = asmName + (cproject?".vcxproj":".csproj");
                                    ns = "NS" + asmName.Substring(0, 1).ToUpper() + asmName.Substring(1);
                                    break;
                                case 'n': if (len > 2) ns = anArg.Substring(2).Trim(); else { ns = args[i + 1]; i++; } fixNS = false; break;
                                case 'D': doDevExpress = true; break;
                                case 'v': if (len > 2) version = anArg.Substring(2).Trim(); else { version = args[i + 1]; i++; } break;
                                case 't': if (len > 2) atype = anArg.Substring(2).Trim(); else { atype = args[i + 1]; i++; }
                                    switch (atype) {
