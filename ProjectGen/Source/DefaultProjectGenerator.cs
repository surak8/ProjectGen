	...
	static class DefaultProjectGenerator {
		...
		static void generateCommonFiles(Project p, PGOptions opts) {
			...
			if (createConfig && !opts.simplyProject) {
				createAppCfgFile(APP_CFG, opts.usePhibroStyle);
			...
			if (createAsmInfo) {
				using (CodeDomProvider cdp = new CSharpCodeProvider()) {
					CodeGeneratorOptions opts2 = new CodeGeneratorOptions();
					...
					CSGenerator.createAsmInfoFile(
						...
						opts.assemblyName, opts.assemblyVersion, cdp, opts2);
		...
		static void generateFiles(Project p, PGOptions opts) {
			CodeGeneratorOptions opts2;
			ProjectItemGroupElement pige;
			CodeDomProvider cdp = new CSharpCodeProvider();

			opts2 = new CodeGeneratorOptions();
			opts2.BlankLinesBetweenMembers = false;
			opts2.ElseOnClosing = true;
			pige = p.Xml.AddItemGroup();
			pige.Label = "IDK";

			if (opts.projectType == ProjectType.XamlApp) {
				XamlGenerator.generateFiles(p, opts, pige, cdp, opts);
			} else {
				CSGenerator.generateMainFiles(p, opts, pige, cdp, opts2);
			}
		}
		...
		static void createAppCfgFile(string filename, bool phibroStyle) {
			...
			using (XmlWriter xw = XmlWriter.Create(filename, xws)) {
				...
				xw.WriteAttributeString("assemblyVersion", "v4.0");
