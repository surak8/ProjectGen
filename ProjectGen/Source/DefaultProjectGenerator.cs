...
namespace NSprojectgen {
    static class DefaultProjectGenerator {
        ...
        static void doStuff(Project p, string szVersion, string asmName, string ns, ProjectType type, bool generate, bool phibroStyle, bool devExpress, bool simple, bool generateCode) {
            ...
            addPropertyGroup(p, true, asmName, type);
            addPropertyGroup(p, false, asmName, type);
            ...
            if (generateCode) {
                generateCommonFiles(p, szVersion, asmName, simple, phibroStyle);
                if (!simple) {
        ...
        static void generateCommonFiles(Project p, string szVersion, string asmName, bool simple, bool phibroStyle) {
            ProjectItemGroupElement pige = null;
            string asmInfoName;

            if (createConfig && !simple) {
                createAppCfgFile(APP_CFG, phibroStyle);
        ...
        static void generateFiles(Project p, string asmName, string ns, ProjectType type, bool devExpress) {
            ...
            if (type == ProjectType.XamlApp) {
                XamlGenerator.generateFiles(p, asmName, ns,pige,cdp,opts);
            } else {
                CSGenerator.generateMainFiles(p, asmName, type, pige, cdp, opts, ns, devExpress);
            }
        }

        static void createAppCfgFile(string filename, bool phibroStyle) {
            ...
            using (XmlWriter xw = XmlWriter.Create(filename, xws)) {
                ...
                xw.WriteAttributeString("version", "v4.0");
                xw.WriteAttributeString("sku", ".NETFramework,Version=v4.5.2");
                xw.WriteEndElement();
                xw.WriteEndElement();
                if (phibroStyle) {
        ...
        static void addDefaultProperty(Project p, ProjectType type, string ns, string aName) {
            ...
            p0.AddProperty("TargetFrameworkVersion", type == ProjectType.XamlApp ? TARGET_452 : TARGET_40);
